...
#6:2:2 로 나누고 싶으면, training을 두번 split하면 됨. or 6:4로 하고, 4를 다시 5:5 로 나눔. 

#x-val 이라는 data를 추가로 넣어줌. model.fit에서 validation_data 추가해줌. 
# 1. 데이터 구성
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.model_selection import KFold #validation data split을 위함. 
...
x_train, x_test, y_train, y_test = train_test_split(xxx, yyy ,test_size = .40, random_state = 66)
#default: 75%:25%
# test_size=0.20
...
#train_test_split() 의 기능: random이 가능함. 하지만, x_train, y_train은 쌍으로 같은 위치로 옮겨짐. 
#random_state = 66 처럼 한번 결정된 경우, 바뀌지 않음.
...
model.fit(x_train, y_train, epochs=73, batch_size=2) 
